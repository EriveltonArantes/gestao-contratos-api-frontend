...
const apiBaseUrl = "https://gestao-contratos-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Parte</li>
          <li>✓ Controle completo de Contrato</li>
          <li>✓ Controle completo de Aditivo</li>
          <li>✓ Controle completo de Ocorrencia</li>
        </ul>
      </div>
...
function PainelParte({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["tipo", "nome", "cpfCnpj", "email", "telefone", "endereco", "cidade", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/partes", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/partes/" + editId : apiBaseUrl + "/api/partes";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["tipo", "nome", "cpfCnpj", "email", "telefone", "endereco", "cidade"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/partes/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "Parte")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>nome</b><span>{item["nome"] ?? "—"}</span></div>
            <div className="item-field"><b>cpfCnpj</b><span>{item["cpfCnpj"] ?? "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Parte</h3>
      ...
      <label className="field-label">cpfCnpj</label>
...
function PainelContrato({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [parteList, setParteList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["partePrimeiraId", "parteSegundaId", "tipoContrato", "objeto", "dataInicio", "dataFim", "valor", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/contratos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/partes", { headers: authHeaders(token) }).then(r => r.json()).then(setParteList).catch(() => {});
  }, [token]);
  ...
  const montarCorpo = () => {
    const corpo = { ...form };
    ["partePrimeira", "parteSegunda"].forEach(k => {
      corpo[k + "Id"] = corpo[k] ? Number(corpo[k]) : null;
      delete corpo[k];
    });
    // Bug real (2026-06-26): <input type="datetime-local"> envia "YYYY-MM-DDTHH:mm"
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/contratos/" + editId : apiBaseUrl + "/api/contratos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["partePrimeiraId", "parteSegundaId", "tipoContrato", "objeto", "dataInicio", "dataFim", "valor", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["partePrimeira", "parteSegunda"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/contratos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["tipoContrato"] ?? "Contrato")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/contratos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipoContrato"] ?? "Contrato")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>partePrimeiraId</b><span>{item["partePrimeiraId"] ?? "—"}</span></div>
            <div className="item-field"><b>parteSegundaId</b><span>{item["parteSegundaId"] ?? "—"}</span></div>
            <div className="item-field"><b>objeto</b><span>{item["objeto"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>partePrimeira</b><span>{item.partePrimeiraId ? ((parteList.find(o => o.id === item.partePrimeiraId) || {}).tipo ?? ("#" + item.partePrimeiraId)) : "—"}</span></div>
            <div className="item-field"><b>parteSegunda</b><span>{item.parteSegundaId ? ((parteList.find(o => o.id === item.parteSegundaId) || {}).tipo ?? ("#" + item.parteSegundaId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Contrato</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">partePrimeiraId</label>
      <input type="number" value={form["partePrimeiraId"] ?? ""} onChange={e => setForm({...form, partePrimeiraId: e.target.value})} />
      <label className="field-label">parteSegundaId</label>
      <input type="number" value={form["parteSegundaId"] ?? ""} onChange={e => setForm({...form, parteSegundaId: e.target.value})} />
      <label className="field-label">tipoContrato</label>
      <select value={form["tipoContrato"] ?? "CLT"} onChange={e => setForm({...form, tipoContrato: e.target.value})}>
      ...
      <label className="field-label">objeto</label>
      <input type="text" value={form["objeto"] ?? ""} onChange={e => setForm({...form, objeto: e.target.value})} />
      ...
      <label className="field-label">partePrimeira</label>
      <select value={form["partePrimeira"] ?? ""} onChange={e => setForm({...form, partePrimeira: e.target.value})}>
        <option value="">Selecione...</option>
        {(parteList || []).map(o => (<option key={o.id} value={o.id}>{o.tipo ?? ("#" + o.id)}</option>))}
      </select>
      <label className="field-label">parteSegunda</label>
      <select value={form["parteSegunda"] ?? ""} onChange={e => setForm({...form, parteSegunda: e.target.value})}>
        <option value="">Selecione...</option>
        {(parteList || []).map(o => (<option key={o.id} value={o.id}>{o.tipo ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
...
function PainelAditivo({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [pagina, setPagina] = React.useState(1);
  const [modoKanban, setModoKanban] = React.useState(false);
  const [contratoList, setContratoList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["contratoId", "numero", "tipo", "descricao", "novoValor", "novaDataFim", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/aditivos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/contratos", { headers: authHeaders(token) }).then(r => r.json()).then(setContratoList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/aditivos/" + editId : apiBaseUrl + "/api/aditivos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["contratoId", "numero", "tipo", "descricao", "novoValor", "novaDataFim", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["contrato"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/aditivos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["numero"] ?? "Aditivo")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/aditivos/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/aditivo" + item.id + "/320/200"} alt="Aditivo" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">🗂️</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["numero"] ?? "Aditivo")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>contratoId</b><span>{item["contratoId"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>novoValor</b><span>{item["novoValor"] != null ? Number(item["novoValor"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>novaDataFim</b><span>{item["novaDataFim"] ? new Date(item["novaDataFim"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>contrato</b><span>{item.contratoId ? ((contratoList.find(o => o.id === item.contratoId) || {}).tipoContrato ?? ("#" + item.contratoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.novoValor)}>Cobrar via Pix</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Aditivo</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">contratoId</label>
      <input type="number" value={form["contratoId"] ?? ""} onChange={e => setForm({...form, contratoId: e.target.value})} />
      ...
      <label className="field-label">novoValor</label>
      <input type="number" step="0.01" value={form["novoValor"] ?? ""} onChange={e => setForm({...form, novoValor: e.target.value})} />
      <label className="field-label">novaDataFim</label>
      <input type="datetime-local" value={form["novaDataFim"] ?? ""} onChange={e => setForm({...form, novaDataFim: e.target.value})} />
      <label className="field-label">status</label>
      <select value={form["status"] ?? "ATIVO"} onChange={e => setForm({...form, status: e.target.value})}>
        <option value="ATIVO">ATIVO</option>
        <option value="PENDENTE">PENDENTE</option>
      ...
      <select value={form["contrato"] ?? ""} onChange={e => setForm({...form, contrato: e.target.value})}>
        <option value="">Selecione...</option>
        {(contratoList || []).map(o => (<option key={o.id} value={o.id}>{o.tipoContrato ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelOcorrencia({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["contratoId", "tipo", "descricao", "data", "responsavel", "resolucao", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/ocorrencias", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/ocorrencias/" + editId : apiBaseUrl + "/api/ocorrencias";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["contratoId", "tipo", "descricao", "data", "responsavel", "resolucao", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["contrato"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/ocorrencias/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
        ...
        <div className="kanban-board">
          {[...new Set(itensFiltrados.map(i => String(i.status ?? "Sem status")))].map(coluna => (
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["tipo"] ?? "Ocorrencia")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/ocorrencias/" + item.id, {
                      method: "PUT", headers: { "Content-Type": "application/json", ...authHeaders(token) },
                      body: JSON.stringify({ ...item, status: e.target.value }),
                    }).then(carregar);
                  }}>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["tipo"] ?? "Ocorrencia")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>contratoId</b><span>{item["contratoId"] ?? "—"}</span></div>
            <div className="item-field"><b>descricao</b><span>{item["descricao"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>resolucao</b><span>{item["resolucao"] ?? "—"}</span></div>
            <div className="item-field"><b>contrato</b><span>{item.contratoId ? ((contratoList.find(o => o.id === item.contratoId) || {}).tipoContrato ?? ("#" + item.contratoId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Ocorrencia</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">contratoId</label>
      <input type="number" value={form["contratoId"] ?? ""} onChange={e => setForm({...form, contratoId: e.target.value})} />
      ...
      <label className="field-label">resolucao</label>
      <input type="text" value={form["resolucao"] ?? ""} onChange={e => setForm({...form, resolucao: e.target.value})} />
      <label className="field-label">status</label>
      <select value={form["status"] ?? "ATIVO"} onChange={e => setForm({...form, status: e.target.value})}>
        <option value="ATIVO">ATIVO</option>
        <option value="PENDENTE">PENDENTE</option>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Parte");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Gestao Contratos<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Parte" ? " active" : "")} onClick={() => setAba("Parte")}><span className="nav-ico">🗂️</span>Parte</button>
        <button className={"nav-btn" + (aba === "Contrato" ? " active" : "")} onClick={() => setAba("Contrato")}><span className="nav-ico">🗂️</span>Contrato</button>
        <button className={"nav-btn" + (aba === "Aditivo" ? " active" : "")} onClick={() => setAba("Aditivo")}><span className="nav-ico">🗂️</span>Aditivo</button>
        <button className={"nav-btn" + (aba === "Ocorrencia" ? " active" : "")} onClick={() => setAba("Ocorrencia")}><span className="nav-ico">🗂️</span>Ocorrencia</button>
      ...
      <main className="main">
        ...
        {aba === "Parte" && <PainelParte token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Contrato" && <PainelContrato token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Aditivo" && <PainelAditivo token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Ocorrencia" && <PainelOcorrencia token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
